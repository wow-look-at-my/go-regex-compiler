...
- **Compiled one-pass path** (`internal/codegen/onepass.go`, `onepass_emit.go`, `templates_onepass.go`): when the pattern is one-pass (every input rune deterministically selects the next step), `buildCapDFA` constructs a capture-annotated DFA — states are sets of NFA `(consuming-inst, pending-captures, empty-width-gate)` configs, transitions carry the capture-slot writes crossed to reach them — and the `<func>Index` core is emitted as a `switch state` automaton with inline `caps[k] = pos` writes. Empty-width assertions at the match boundaries are compiled: text anchors (`^ $ \A \z`) fold away as always-satisfied for a full match, and a leading/trailing `\b`/`\B` becomes a one-line word-boundary gate on the first/last rune.
- **Compiled TDFA path** (`internal/codegen/tdfa.go`, `tdfa_emit.go`, `templates_tdfa.go`): for patterns with genuine capture AMBIGUITY the one-pass path rejects — adjacent greedy stars (`(a*)(a*)`), overlapping alternation (`(a|ab)(a*)`), nested stars (`(a*)*`), optional-then-star (`(a?)(a*)`), and `(?i)` fold classes — `buildTDFA` determinizes the marker-annotated NFA into a tagged DFA. Each live config owns an isolated block of an integer register file; transitions carry fixed set-to-position / copy-from-source register ops, and leftmost-greedy priority (the same Go's regexp uses) resolves every merge at construction time. The accepting state reads the winning config's register block. Still a pure `switch state` machine — no program table, thread list, epsilon-closure, or `sync.Pool`.

The Thompson NFA-simulation (`templates_submatch.go`) is retained ONLY behind the opt-in `SubmatchOptions.ForceInterpreter` flag as a differential test oracle; the normal generation path never emits it. Interior empty-width assertions (e.g. `foo\bbar`) that neither compiled path handles are rejected by `dfa.ValidateAssertions` (an error), never interpreted.
...
- The TDFA register file is a stack-allocated `[N]int` (N = maxConfigs*numSlots); a
  ...
  hazard-free. Only the oracle path (`ForceInterpreter`) builds the package-level NFA
  program `var <priv>Prog []` (name-mangled so fixtures sharing a package do not collide)
