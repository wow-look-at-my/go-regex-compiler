...
- Case folding expands the FULL Unicode simple-fold orbit (no per-range cap):
  `dfa.ExpandFoldCase`/`dfa.NormalizeRunePairs` are exported and also used by
  the submatch codegen to bake fold orbits into the emitted NFA rune tables
  (a folded single-rune literal becomes an `opRune` range-pair instruction)
...
- Header imports (match/utf8) are gated on the rendered body actually
  containing a matching loop; short-circuit bodies (start-accepts, literal
  `strings.Contains`) emit no unused imports
